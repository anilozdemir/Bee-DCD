...
class ESN_Reservoir(nn.Module):
    ...
    def __init__(self, nInput: int, nOutput: int, nReservoir: int, **kwargs):
        ...
        self.repeat      = kwargs.get('repeat'    , 30)    
    ...
    def forward(self, x):
        ...
        if not x.shape[0] == 1:
            raise ValueError('Wrong input format! Shape should be [1,N]')
        x = x.to(self.device)
        X = torch.stack([self.update_leakyIF(x[0]) for _ in range(self.repeat)]) # show the same stimuli multiple times
...
class softMax(nn.Module):
    def __init__(self, temp = 1):
        ...
        self.temp = temp
    def forward(self, x):
        return nn.Softmax(dim=1)(x*self.temp)
